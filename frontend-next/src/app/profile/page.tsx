...
export default function ProfilePage() {
  ...
  useEffect(() => {
    api.get('/auth/me')
      .then((r) => { setUser(r.data); setForm({ full_name: r.data.full_name || '', phone: r.data.phone || '' }) })
  ...
  const save = async (e: React.FormEvent) => {
    ...
    setMsg({ text: '', type: '' })
    try {
      await api.patch(`/admin/users/${user.id}`, { full_name: form.full_name || null, phone: form.phone || null })
  ...
  return (
    <div className="auth-page">
      <button className="auth-theme-toggle" onClick={toggle}>{theme === 'dark' ? 'Light Mode' : 'Dark Mode'}</button>
      <div className="auth-card" style={{ maxWidth: '480px' }}>
        ...
        <form onSubmit={save}>
          ...
          <div className="form-group">
            ...
            <input className="form-input" type="tel" value={form.phone} onChange={(e) => setForm((f) => ({ ...f, phone: e.target.value }))} placeholder="10-digit mobile number" />
          </div>
          <button className="btn-submit" type="submit" disabled={saving}>{saving ? 'Saving...' : 'Save Changes'}</button>
        </form>
        <div style={{ marginTop: '1.25rem', display: 'flex', gap: '0.75rem' }}>
          <button className="btn-secondary" onClick={() => router.back()} style={{ flex: 1 }}>Back</button>
          <button className="btn-secondary" onClick={() => { clearAuth(); router.replace('/login') }} style={{ flex: 1 }}>Sign Out</button>
        </div>
      </div>
    </div>
  )
